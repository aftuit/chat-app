  ...
  <li class="message-content flex items-end gap-1" :class="{ 'flex-row-reverse': isCurrentUser }">
    <div class="w-9 h-9">
      <div v-if="!isAgainAssigned" class="rounded-full text-center text-2xl bg-green-300 h-full w-full">
        {{ message.sender_id == 25 ? "T" : "P" }}
      </div>
    </div>
    <div class="border py-1 px-1 rounded-xl bg-gray-100 w-fit max-w-[300px] text-wrap items-end">
      <div class="leading-5 truncate px-3">{{ message.message }}</div>
      <div class="flex justify-end items-center gap-1">
        <span class="text-gray-400 text-sm float-end">{{ formatDate(message.created_date) }}</span>
        <Loader v-if="message.loading" />
        <span v-else>
          <svg width="20" height="20" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg" v-if="message.is_read">
        ...
        <!-- <span>
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="ms-1 cursor-pointer">
              <path
...
// Have no any url to get user information from server so I saved it manually in LocalStorage
